...
declare function setup:do-setup($import-config as element(configuration)) as item()*
{
  try
  {
    ...
    try {setup:create-roles($import-config)} catch($ex) {},
...
declare function setup:appserver-set-privilege2(
...
<database>{$default-security}</database>
  ...
  (
    ...
    if (setup:get-roles(())/sec:role[sec:role-name = $role-name]) 
    then fn:concat("Role ",$role-name, " already exists")
    else
    let $role-id := (
      xdmp:eval(
       'import module namespace sec="http://marklogic.com/xdmp/security" at "/MarkLogic/security.xqy";
...
sec:user-set-default-collections($user-name, $collections/*)',
    ...
    return fn:concat("User Created",$user-name," with roles: ", fn:string-join($role-names,","))
    else
    (
      xdmp:eval(
        'import module namespace sec="http://marklogic.com/xdmp/security" at "/MarkLogic/security.xqy";
